...
app.get('/show', (req, res) => {
    ...
    db.collection('cliente').find().sort({ name: 1 }).toArray(function (err, results) {
        ...
        db.collection('pastel').find().sort({ name: 1 }).toArray(function (error, resultado) {
            if (error) return console.log(error)

            res.render('show_all.ejs', { data: results, dados: resultado })
        })
    })
})
...
app.post('/pedido', async function (req, res) {
    ...
    console.log(obj)
    const cliente = await db.collection('cliente').findOne(ObjectId(obj.cliente))

    console.log(cliente)

    const listaDePasteis = []
    for (var j = 0; j < obj.pasteis.length; j++) {
        let pastel = obj.pasteis[j]

        var pastelDoBanco = await db.collection('pastel').findOne(ObjectId(pastel.id))
        console.log(pastelDoBanco)
        for (var i = 0; i < pastel.qtde; i++) {
            listaDePasteis.push(pastelDoBanco)
        }
    }


    db.collection('pedido').insertOne({
        cliente: cliente,
        pasteis: listaDePasteis,
